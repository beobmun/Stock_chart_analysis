...
class Model:
    ...
    def train2(self, epsilon_init, epsilon_min, epochs, batch_size, learning_rate, transaction_penalty, gamma, train_s, train_e, val_s, val_e, data_dir="../get_data/data/US/chart_image", save_dir="results"):
        epsilon = epsilon_init
        num_actions = self.model.num_actions
        optimizer = optim.Adam(self.model.parameters(), lr=learning_rate)
        criterion = torch.nn.SmoothL1Loss()
        ...
        for epoch in range(epochs):
            ...
            try:
                ...
                train_dataset = Dataset2(self.train_df_cache, num_days=20, start_date=train_s, end_date=train_e, transform=transform, data_dir=data_dir)
                ...
                with tqdm(total=len(self.train_df_cache), desc=f"Training Epoch {epoch + 1}/{epochs}", ncols=100, leave=False) as pbar:
                    for imgs_batch, yield_batch in train_dataloader:
                        for b in range(imgs_batch.shape[0]):
                            # 전체 date 길이 만큼 이미지
                            chart_imgs = imgs_batch[b].to(device)
                            yields = yield_batch[b].to(device)
                            
                            pre_states = chart_imgs[:, 0, :, :, :]
                            cur_states = chart_imgs[:, 1, :, :, :]
                            next_states = chart_imgs[:, 2, :, :, :]
                                                        
                            if round(random.uniform(0, 1), 4) < epsilon:
                                pre_actions = self._get_random_action(num_actions, chart_imgs.shape[0]).to(device)
                                cur_actions = self._get_random_action(num_actions, chart_imgs.shape[0]).to(device)
                            else:
                                with torch.no_grad():
                                    _, pre_actions = self.model(pre_states)
                                    _, cur_actions = self.model(cur_states)
                            
                            cur_rewards = self._get_reward_neutral(pre_actions, cur_actions, yields, transaction_penalty, scale=scale, cap=cap, neutral=neutral)
                            for i in range(chart_imgs.shape[0]):
                                self.memory.push(
                                    cur_states[i].squeeze(0),
                                    cur_actions[i].unsqueeze(0),
                                    cur_rewards[i],
                        ...
                        if len(self.memory) < (batch_size*cur_states.shape[0]):
                            pbar.update(1)
                            continue
                        
                        states, actions, rewards, next_states, y_t = self.memory.get_random_sample(batch_size*cur_states.shape[0])
                        states = states.to(device)
                        actions = actions.squeeze(1).to(device)
                        rewards = rewards.to(device)
                        next_states = next_states.to(device)
